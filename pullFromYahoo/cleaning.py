...
def cleanPlayerData(d,i):
    d   = d['fantasy_content']['team'][1]['roster']['0']['players'][str(i)]['player'] # strip the nonsense

    d0  = d[0] + [{'selected_position': d[1]['selected_position'][1]['position']}] # this makes a list entry     
    d0.append(dict(d[1])) # append the other two that are correct 
    if len(d) > 2:
        d0.append(dict(d[2]))   # for some reason they add an "is_editable" field for the current user
